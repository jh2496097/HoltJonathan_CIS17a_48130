...
int main(int argc, char** argv) {
    ...
    //showTests (scores, testScores);
           ...
           cout << fixed << setprecision(1) << "The average of the test scores "
                   "is " << ": " << avg <<endl << endl;
 
           avg2 = newAvg (scores, testScores);
           cout << fixed << setprecision(1) << "The average of the test scores "
                   " minus lowest score is " << ": " << avg2 <<endl << endl;
...
/*
 * This function looks a list of scores entered by user, It finds the lowest
 * test score in the list and drops it then does the avg.
 */
float newAvg (int *p, int size)
{   
    int newSize = size -1;
    //setting new ptr with newSize because of dropped score
    int *copyPtr = new int [newSize];
    //since already sorted just copying to new ptr + 1 to ignore lowest score
